/*
 * @Author: Quarter
 * @Date: 2022-08-26 17:00:52
 * @LastEditors: Quarter
 * @LastEditTime: 2022-08-26 17:04:34
 * @FilePath: /vite-plugin-svg/src/svgo.ts
 * @Description: svgo 处理
 */
 ...
 * @param {string} raw 自定义配置
 * @param {SVGOConfig} config 自定义配置
 * @return {OptimizedSvg|OptimizedError}
 */
export const svgo = (raw: string, config?: SVGOConfig) => {
  const options = getSVGOOptions(config);
  return optimize(raw, options);
};
