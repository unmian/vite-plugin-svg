...
const normalizeAttrs = (
  ...
  return {
    style: properties && properties.style ? normalizeStyle(String(properties.style)) : {},
    ...attrs,
  };
};
...
const astToElement = (nodes: Node[], options: SvgToElementOptions): IconElement[] => {
  const elementNodes = nodes.filter((node) => node.type === "element") as ElementNode[];
  return elementNodes.map((node) => {
    ...
    if (Array.isArray(node.children)) {
      children = node.children.filter((child: any) => typeof child !== "string") as Node[];
    }
 ...
 */
export const svgToElement = (
  raw: string,
  options: SvgToElementOptions = { replaceColor: false },
) => {
  const ast = parse(raw);
  ...
  if (attrs) {
    ...
    if (styleObj) {
      const styleStr = Object.keys(styleObj).reduce((styles, styleKey) => {
        ...
        destStyle += `${Word.toSeparatorCase(styleKey, { separator: "-" })}: ${
          styleObj[styleKey]
        };`;
        return destStyle;
      }, "");
